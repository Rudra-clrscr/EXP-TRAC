...
import { ArrowDown, BarChart2, ChevronDown,TrendingUp as ProfitIcon,PieChart as PieChartIcon, ChevronUp, DollarSign, PiggyBank, Plus, ShoppingCart, TrendingDown, TrendingUp, Wallet } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const prevTimeFrameRange = useMemo(() => getPreviousTimeFrameRange(timeFrame), [timeFrame]);
    ...
    //to check if a date is in range or not
  ...
  //to update gauge when time frame changes
  useEffect(() => {
    const maxValues = {
 ...
 {/* Expense distribution pie - Hidden on mobile */}
      ...
      <div className={dashboardStyles.listsGrid}>
        ...
        <div className={dashboardStyles.listContainer}>
          <div className={dashboardStyles.listHeader}>
            <h3 className="text-lg md:text-xl lg:text-xl xl:text-xl font-bold text-gray-800 dark:text-gray-200 md:mt-3 mt-3 flex items-center gap-3">
